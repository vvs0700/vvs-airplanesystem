--Job Script edited by vvs#0700 and AndroX23#9971
...
AddEventHandler('vvs_pilotjob:hasEnteredMarker', function(marker)
...
AddEventHandler('vvs_pilotjob:hasExitedMarker', function()
	CurrentAction = nil
...
RegisterNetEvent('vvs_pilotjob:startDeliveryJob')
AddEventHandler('vvs_pilotjob:startDeliveryJob', function()
end)
	...
	while true do Wait(0)
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pilot' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) then
				...
				if jobMarker then
					TriggerEvent('vvs_pilotjob:hasEnteredMarker', 'job')
				elseif clothesMarker then
					TriggerEvent('vvs_pilotjob:hasEnteredMarker', 'clothes')
				elseif vehicleSpawner then
					TriggerEvent('vvs_pilotjob:hasEnteredMarker', 'vehicle')
				elseif vehicleDeleter then
					TriggerEvent('vvs_pilotjob:hasEnteredMarker', 'deleter')
				end
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('vvs_pilotjob:hasExitedMarker')
			end
		end
	end
end)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if not DoesEntityExist(vehicle) then
				...
				TriggerEvent('vvs_pilotjob:startDeliveryJob')
			else
